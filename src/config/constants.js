...
// 🔴 **ここが W の既定値の唯一の定義。** 各ページの `defaultOptions` は必ずこれを読むこと。
//    2026-08-12 まで main.js / kickPage.js が `dwellMinutes: 10` を直書きしており、
//    **新規利用者に効く W は 17 ではなく 10 だった**（目盛り①の既定10 の名残）。
//    10 は今の目盛りの中に在るので移行も「範囲内＝最寄りへ」で素通りし、無言で残っていた。
//    検査 CR が「両ページの既定が constants の default* と一致すること」を見ている。
...
// 等間隔にするとスライダーの右半分が死ぬ。比 1.2 前後の等比にしてある。
//
// 🔴 **奇数個にすること**（2026-08-11・利用者要望）。偶数個だと**真ん中の目盛りが存在せず**、
//    既定値がスライダーの中央に来ない。既定 17分 は必ず**ちょうど中央**（添字 4／全9段）に置く。
//    検査 CN が「奇数個」「中央＝既定値」の両方を見ている。
// ⚠️ **既定値を必ず含めること。** 含めないと既存ユーザーの値が最寄りへ丸められる。
//
// 経緯（合わせ込みの目盛りであって、大きく動かすものではない）:
//   [3,5,8,10,14,20,30,45]（既定10）        … 全体に短すぎた
//   [10,17,27,40,58,80,110,150]（既定40）   … 広すぎた。実機比較で 17分 が一番近いと判明
//   [11,13,15,17,19,22,25,29]（既定17）     … 今度は**狭すぎた**。しかも 17 が中央でなかった
//   [8,10,12,14,17,20,24,29,35]（既定17）   … 中央を作り、両端へ均等に広げた（現在）
//
// ✅ **2026-08-12 に利用者が再確認済み。** 推定同接の式を作り直した（項目CQ）ので 17分 の根拠を
//    引き継げるか怪しかったが、新しい式でも**スライダー中央（17分）でちょうどよい**と確認された。
//    ここは合わせ込みの目盛りなので、実機比較の裏付け無しに動かさないこと。
export const dwellMinutesScale = [8, 10, 12, 14, 17, 20, 24, 29, 35];
