...
function computeSignature(img) {
    ...
    try {
        ...
        // ここへ来る＝**汚染画像を渡された**（doc/09 項目CV）。①が守るべき約束が破れている。
        //
        // 🔴 **汚れた canvas を捨てること。** `sigCanvas` は使い回しで、一度でも汚染画像を描くと
        //    **その canvas は永久に汚染される**。捨てないと、原因が直った後もきれいな画像で
        //    例外が出続け、「直したのに直らない」になる（この後 captureUnsupported で止めるので
        //    すぐには効かないが、汚れた状態を黙って抱え込まないこと）。
        sigCanvas = null
        sigCtx = null
        // 1回だけ警告（コンソールのコンテキスト選択に関係なく top にも表示される）。
        // ⚠️ **「通常は起きない」で済ませないこと。** 2026-08-12 に実際に出て、原因は
        //    kick.com の画像中継が失敗した時の**素のURLへのフォールバックを給餌していた**こと。
        //    文面に心当たりを書いておく（調べる人を空振りさせない）。
        if (!captureUnsupported) {
            console.warn(
                '⚠️ 動くサムネ: 汚染された画像を渡されたので、このページではコマ化を止めます（表示は維持）。'
                + ' 呼び出し側が crossOrigin か中継(data URL)以外で読んだ画像を渡しています'
                + '（doc/09 項目CV）。ページを開き直すと復帰します。'
            )
        }
