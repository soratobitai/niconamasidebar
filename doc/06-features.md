...
- 盛り上がり: 直近の増分レートのEMA（τ=3分・`utils/momentum.js`。計算は storage の upsert）。`applyRankAttributes` が `active-point` 属性（と同点用の `data-total`）へ書き込み、`sortPrograms` の `active` がそれを降順に並べる。
