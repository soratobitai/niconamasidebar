...
### 🔴 既定値を変えても、すでに使っている利用者には届かない

`storage.getOptions` は `{ ...defaultOptions, ...stored }` をマージした結果を**その場で
storage へ書き戻す**（`pickOptionKeys` で `optionKeys` のキーだけ）。つまり**そのキーを持つ版を
一度でも動かした時点で、既定値が利用者の storage へ焼き付く。** 以後は `stored` が勝つので、
既定を変えて効くのは次の2種類だけ。

- 新規インストールの人
- そのキーがまだ無かった頃の版から、一気に更新した人

⚠️ **「新機能に気付いてもらうために既定をONにする」は成立しない**（2026-08-13 に検討して却下）。
**届いてほしい既存利用者にだけ届かない**という、いちばんかみ合わない形になる。
告知はウェブストアの更新内容か、設定側に印を付ける形で行うこと。
→ 2026-08-17 に実際にやった形が **doc/09 項目DA**（Kick連携のお知らせ）。**保存キーは
`optionKeys` に入れていない**（設定ではなく一度きりのUI状態。入れると上の焼き付きに巻き込まれる）。

⚠️ 逆に、`optionKeys` に**載せていない**キーは焼き付かないので、既定を変えると全員に効く。
  既定値を動かす前に、そのキーが `optionKeys`（`config/constants.js`）に在るか必ず見ること。
  実測（2026-08-13）: `defaultOptions` の14件は**全部** `optionKeys` に在る＝今はどれも焼き付く。
