...
## ✅ AD. リスト取得を notifybox からフォローAPIへ一本化（R-3・2026-07-29）

**2系統だった取得を1系統にした。** 旧構成は「リスト＝notifybox／詳細＝フォローAPI」を `Promise.all` で並列取得し、**notifybox の返却順を新着順の基準**にしていた。

### 撤去できた理由

notifybox が返すのは実質 id と title だけで、**フォローAPI は同じ番組集合に加えて `beginAt`（放送開始時刻）まで返す**。新着順の定義は「放送開始が新しい順」なので、`beginAt` 降順で同じ並びが作れる。

> notifybox に依存していた本来の理由は「**lv番号は予約/作成順で採番されるため放送開始順とズレる**」だった（項目 R）。これは正しい指摘だが、解決策としてAPIの並び順に頼る必要はなく、**`beginAt` を直接使えば済む**。

### 実測（2026-07-29・ログイン済み実データ）

| 検証 | 結果 |
|---|---|
| 番組の集合 | notifybox 13件 / フォローAPI 13件 / **差分 0件** |
| 並び順 | **`beginAt` 降順で notifybox の並びと完全一致**（13件・食い違い0） |
| `beginAt` の欠落 | 0件 |
| 新着検知の速さ | **フォローAPIの方が速い**（実測: 同一番組で notifybox より 100秒 早く出現。チャンネル番組ではフォローAPIにのみ出現する例もあった） |

### 🔴 同時に解消した実害：表示が100件で頭打ちだった

```
フォローAPI: 最大500件の詳細を取得  ──┐
                                      ├→ notifybox に載る100件だけ表示されていた
notifybox:  rows=100・ページング無し ──┘
```

**カードを作る元が notifybox 側だったため、放送中のフォロー番組が100件を超えると101件目以降が表示されなかった。** 詳細だけは取得していたのでその通信は完全な無駄。項目 U には「約70件で頭打ちの制約は解消」とあるが、**解消していたのは詳細取得だけで表示は100件のままだった**。

### 実装

- `_refreshDetailsViaScrape()` が取得結果を返すようにし、これがリストと詳細の**両方**の情報源になる
- `_orderByBeginAtDesc()` が新着順の基準を作る。**同時刻は lv番号の降順**で決定的にする（安定ソート＝毎周期で順序が揺れない）
- `data-api-index` の意味が「notifybox の返却位置」から「**beginAt 降順での位置**」に変わった。`sorting.js` の `newest` はこの属性を読むだけなので実装は不変
- `#api_error`（ログイン誘導）の切替は `updateSidebar` へ移動。フォローAPIは**失敗時 `null`・成功時は配列**（0件でも配列）を返すので、「取得失敗」と「放送中0件」を区別できる
- カード生成は取得結果の配列から直接行う（storage への書き戻し→読み直しの往復をやめた）
- 削除: `services/api.js` の `fetchLivePrograms`、`UpdateManager.getLivePrograms`、`constants.notifyboxAPI`
...
| 失敗パターン | 「片方だけ失敗」がある | 単一障害点のみ |
| 突合ロジック | 2つの結果を id で照合 | 不要 |

> ⚠️ **カードのDOM id は「lv を外した数値」の規約を維持している。** サムネ更新側が `` infoMap.get(`lv${card.id}`) `` で引き直す前提になっているため、ここを変えるとサムネ更新が全滅する。

> **退行検出**: `npm run verify:e2e` に「撤去済みの notifybox API が一度も呼ばれない」チェックを入れてある。
