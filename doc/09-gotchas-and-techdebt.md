...
よく見る配信者ほど上に出す。**点数制**（開いた時+1／視聴12分ごと+1・1回の視聴で最大+5）。
数えるのは**配信者**。ニコ生は放送ごとに lv が変わるので、番組で数えても次の放送に効かない。

⚠️ **満点までの時間は「刻み × 上限」で決まる。** 片方だけ動かすと黙って変わるので、
その積を検査で固定してある（**60分で満点**。2026-08-11 に利用者判断で 25分 → 60分）。
...
## ✅ CN. 「人気順の基準」の目盛りを校正した（2026-08-10 に2度・2026-08-11 に1度）
