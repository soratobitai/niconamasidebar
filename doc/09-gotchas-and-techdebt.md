...
⚠️ **④の時点で 17分 という値の根拠は失われている。** 2026-08-11 に推定同接の式を
作り直した（項目CQ）ため、17分は**旧実装の偏りを平均で埋め合わせた値**であって
持ち越せない。再校正は実機比較の途中。
...
**「—」が出る場合はまだ1つある。** notifybox だけが知っている新着番組は**来場者数そのものが
未取得**（`viewers: 0`）なので、フォローAPIが拾うまで「—」になる。通常20〜101秒、通信が悪いと
その間ずっと。これは「推定できない」ではなく「**まだ何も分かっていない**」なので、
数字を作らずに「—」のままにしてある。詳細APIで埋めるなら別項目として起こすこと。

**目盛り（W）は校正し直しが要る。** 値が全体に上がり、しかも上がり幅がカードごとに違っていた
（観測が短いカードほど大きく上がる）。項目CN の 17分 は**旧実装の偏りを平均で埋め合わせた値**
なので、そのまま持ち越せない。
...
## 🔴 CM-2. テスト用の表示（**撤去予定**・2026-08-10）

実機で数字を確かめるために出しているだけ。**確認が済んだら消すこと。**

| 表示 | 場所 | 消す箇所 |
|---|---|---|
| 視聴点数（`7pt`） | 配信者名の行の右端 | `applyRankAttributes` の2行 / `makeProgramElement` のバッジ生成 / `main.css` の `.watch_count_badge` |
| 同時視聴者数 | サムネ左上 | `applyRankAttributes` の2行 / `makeProgramElement` の生成 / `main.css` の `.viewer_overlay` |
