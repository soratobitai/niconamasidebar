...
## ⏸ R-2（メモリモデル導入）は未着手のまま — 判断待ち

2026-07-29 の夜間作業は**ここで止めた**。R-2 は「いま画面にどの番組が居るか」をメモリ上の配列で持ち、DOM をその投影にする改修で、8段階の計画と**10件の禁止事項**が調査で出ている。

### 止めた理由

1. **数時間前に「見送り」を推奨したまま、判断が確定していない。** その後 FLIP を入れて評価は好転したが（下記）、決めたのは私ではなく利用者であるべき。
2. **段階1〜2は準備であって、単独では利用者に見える価値がない。** 一方で「記録とDOMの食い違い」という新しい失敗経路を作る。
3. 夜間の作業で**検証スクリプト側のミスを4回**出している（固定幅スライス3回・自己矛盾1回）。監視のない状態で、価値が後払いの改修に入るのは分が悪い。

### FLIP を入れたことで変わった評価

**FLIP は R-2 の最大の失敗を可視化するカナリアになる**（項目AI）。

| R-2 の禁止事項 | FLIP 無し | FLIP 有り |
|---|---|---|
| `_sortOrderChanged` をモデル同士の比較にする | レイアウト再計算が余分に走るだけで**ほぼ見えない** | **全カードが毎周期スライド**して即バレる |
| 描画を非同期化する | サムネが無言で止まる | **アニメが壊れて見える**（＋項目AL の警告が鳴る） |

「検証が効かない領域に入るのは危険」という懸念は**部分的に解消している**。

### 再開する場合の入口

`R-2 調査の段階計画`（8段階）の段階1から。**各段階は単独でコミットでき、その時点で全機能が動くこと**が必須条件。特に:

- モデルは**描画した結果**を書く（取得結果の代入にしない）。早期returnが3本あるため
- `_sortOrderChanged` は「今DOMに出ている順」対「あるべき順」のまま。モデル同士の比較にしない
- カードを作り直さない・コンテナのノードを差し替えない・描画を非同期化しない
- `stopAllTimers` / `cleanup` からモデルをクリアしない
...
|---|---|---|
| **人気順**（`active`） | **動く**。`active-point = (視聴者+コメント) / 経過分` で、分母が番組ごとに違う速さで増えるため | **ほぼ毎周期** |
