...
## BF. 番組が終了してもリストから消えなかった（2026-08-01 修正）

フォローAPI（`?status=onair`）は終了した番組をしばらく返し続ける。リストの削除判定は
「和集合に居ない番組を消す」なので、フォローAPIが手放すまでカードが残っていた。

**notifybox のほうが反映が早い**（利用者の認識・経験則）。そこで notifybox の不在を終了の合図に使う。

### 🔴 不在は、それだけでは終了の証拠にならない

`UpdateManager._dropEndedByNotifybox()` が3つの条件を全て満たす時だけ根拠にする。

| 条件 | 外すと何が起きるか |
|---|---|
| notifybox の取得が成功している（失敗時 `false`） | **通信断で全番組が消える**。一番やってはいけない壊れ方 |
| 応答が**あふれた疑い**でない（下記の2本立て） | あふれただけの番組を「終了」と誤判定する |
| その番組が**以前 notifybox に載っていた** | notifybox の守備範囲外の番組が、**出た瞬間に消える** |

### あふれ判定は2本立て（真の上限が分からないため）

```js
if (notifyList.length >= currentNotifyboxRows()) return programs;  // (a) 要求数ぴったり
if (notifyList.length === notifyboxKnownCap) return programs;      // (b) 実績値ちょうど
```

`rows` はこちらが投げるパラメータで、**APIの真の上限は不明**。初回コミットからずっと 100 が
入っていたが根拠の記録はどこにも無く、100件超の放送中を作れないので実測もできない。
そこで**上限を知らないまま両方に備える**: 要求は 500 で出し、(a)(b) の両方で守る。

| 実際の上限 | 500要求した結果 | 判定 |
|---|---|---|
| 100 だった | 100件返る | (b) が捕まえる → 消さない ✅ |
| 500 いける | 150件返る | どちらにも当たらない → 消してよい ✅ |

🔴 **(b) を `>=` で書かないこと。** 上限が本当に 500 だった場合、150件の応答が `150 >= 100` で
引っかかって**終了検知が常に止まる**。「ちょうど一致」で見る（上限が500で放送中がちょうど
100件の時だけ1周期空振りするが実害は無い）。`verify:loop` に **`>=` なら落ちる項目**を置いてある。

### `rows=500` を拒否するAPIだった場合の保険

`fetchLivePrograms` は **meta.status が異常な応答**を受けたら、以後の要求件数を実績値(100)へ落とす。
落とさないと notifybox が永久に死に、新着検知が 20〜101秒 遅くなる（エラーは1回出るだけ）。

⚠️ **通信断の catch から落とさないこと。** ネットが一瞬切れただけで以後ずっと少ない件数になる。
**実装中に実際に踏んだ** — 検証で「通信失敗」を1回起こした時点で以後の要求が100に下がり、
別の検証項目が落ちて発覚した。「APIが rows を拒否した」なら応答は返って meta.status が
200 以外になるはずで、区別はそこで付く。

⚠️ 呼び出し側は `notifyboxRows`（定数）ではなく **`currentNotifyboxRows()` を読むこと**。
下がったことに気付かないと、あふれ判定が黙って外れる。

3つ目は `this._notifyboxLive`（notifybox が一度でも載せた番組id）で判定する。
**notifybox の守備範囲をこちらが知らなくても成り立つ**形にしてある
——「notifybox が一度でも面倒を見た番組だけ、notifybox の言い分を信じる」。

### 印は足し込む。置き換えない（実装中に踏んだバグ）

`this._notifyboxLive` を毎周期 `live` で上書きすると、**消した番組の印まで落ちる**。
フォローAPIはその番組をまだ返し続けているので、次の周期には条件3で
「notifybox が知らない番組」に化けて**復活する**。消えて出て、を繰り返す。

足し込みにして、掃除は「notifybox にも今回のリストにも居ない番組」だけにする
（＝フォローAPIも手放した時に落ちる）。`verify:loop` にこの性質を固定してある。

### 承知のうえの副作用: notifybox が失敗した周期は終了済みが一時的に戻る

判定そのものを止めるため、フォローAPIがまだ返している終了番組が1周期だけ復活する。

消したidを永続的に抑止すれば防げるが、**判断が誤っていた場合に生きている番組を永久に隠す**
ことになる。そちらのほうが遥かに悪いので、毎周期評価し直す側を選んだ。

### 鳴る罠

消した番組が notifybox に戻ってきたら `console.warn` する（1回だけ）。
**この判断が誤っていた時の症状は「放送中の番組が黙って画面から消える」で、エラーが一切出ない。**
利用者からは「たまに番組が消える拡張」にしか見えず、原因に辿り着けない種類の壊れ方である。

### 検証（`verify:loop` の `notifyboxEndDetection`）

失敗の方向が非対称（消しすぎ ≫ 消し足りない）なので、**「消えてはいけない時に消えないこと」**を厚く固定している。

各ガードを1つずつ壊して**落ちることを確認済み**（条件1・条件2・印の足し込み・機能そのもの）。
⚠️ **条件3（守備範囲外は消さない）の空振り確認だけは未了。** その行を消すと検証スクリプトが
node ごとクラッシュして、落ちる/落ちないの判定ができなかった。実装の正しさとは別件だが、
**この項目は「落ちることを確かめた検証」ではない**ことを承知して読むこと。

### 🔴 依存している前提（実測はしていない）

**「notifybox は番組が終わるまで載せ続ける（古い番組を流し落とす通知フィードではない）」**

崩れると長時間放送が生きたまま消える。**利用者の経験則により「消えることはない」と確認され、
それを根拠に実装した**（2026-08-01）。API を叩いての実測はしていない。
