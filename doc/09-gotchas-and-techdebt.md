...
notifybox から消えた  →  「終わったかもしれない」（疑い）
                      ...
                      →  meta.status 404（その番組は存在しない）なら消す
                      →  答えが得られなければ消さない
```
...
| 詳細APIが答えない時（通信断・想定外＝`metaStatus 0`）は**消さない** | 判断材料が無いのに消す。BF の失敗を再現することになる |
| **404 は消す**（下の節） | 終わって削除された番組が、フォローAPIが手放すまで残り続ける |
| **疑いが3件以上そろって全員404の周期は1件も消さない** | id の作り方やAPIのパスが変わると全員404になる。信じると**カードが一斉に消える** |
...
### 🔴 404 は「答えが無い」ではなく「その番組は存在しない」という答え（2026-08-17 に修正）

**それまで 404 を通信断と同じ「消さない」側に置いていた。誤りだった。**
利用者から「終わっているのにリストから消えない番組がある（まれ・更新ボタンでも消えず・
ページ更新で消えた）」の報告を受けて見直した。

実測（2026-08-17・ログイン不要の公開API）:

| 確かめたこと | 結果 |
|---|---|
| 放送中の番組は 200 を返すか | **70件すべて 200**（一覧APIの放送中70件を全部叩いた） |
| 終了した番組は 404 に化けるか | **化けない。** 2025年10月・2024年4月・**2012年7月**の番組がどれも `ended` を返す |
| 終了直後に削除される番組はあるか | **60番組を13分追って0件**（11件が終了し、全部 `ended` のまま。母数が小さく「無い」とは言えない） |
| 404 の中身 | `{"meta":{"status":404,"errorCode":"NOT_FOUND","errorMessage":"program not found"}}` |

**放送中なら必ず 200。だから 404 は「放送していない」と読める。**

🔴 **見ているのは応答JSONの `meta.status` であって HTTP のステータスではない。**
ゲートウェイやCDNが返す 404 は HTML なので `.json()` が投げ、`metaStatus 0`（＝答えが無い）へ
落ちる。**この2つを同じ値にしないこと。** 混ぜると通信断で放送中の番組が消える
（検証 `CO ⑩-2` と `BF-2 詳細APIが答えない周期は1件も消さない` の2つが鳴る。実際に壊して確認済み）。

⚠️ **「全員404なら消さない」を外さないこと。** 失敗の方向は非対称（消しすぎ ≫ 消し足りない）で、
404 を消す側に回した以上、`liveInfoAPI` のパスや id の正規化が壊れると**全カードが一斉に消える**。
3件以上そろって全員404なら、番組ではなくこちらを疑う（1回だけ `console.warn`）。
3件未満で判定しないのは、1〜2件だけ終わって消えるのが普通に起きるため。

⚠️ **このガードには承知の副作用がある。** 本当に3件以上が同時に「終了と同時に削除」された場合、
その組は**永久に消えない**（毎周期どれも404なのでガードが立ち続ける）。
ただし**修正前も同じく消えなかった**ので後退ではなく、改善が届かない範囲というだけ。
実測では終了11件中の削除は0件（同日）なので、この形が起きる頻度は低いと見ている。
🔴 **直したくなったら「同じ周期の `fillMissingDetails` が 200 を得ているか」を材料にすること。**
あれも同じ詳細APIを叩くので、200 が返っていればURLと id の作り方は壊れていないと言える。
件数で判断するのをやめられるが、モジュールをまたぐ状態が要る。頻度が低いうちは割に合わない。

🔴 **`meta.status` は数値の時だけ採用する**（`Number.isFinite`）。`Number('404')` のような
緩い変換にすると、想定外の応答が「終了の答え」に化けて**放送中の番組を消す側へ倒れる**。
検査 `BF-2⑫` が、文字列の `'404'` と「200 なのに data が無い」応答の両方で実際に確かめている。

🟢 **404 での誤削除は、下の「鳴る罠」がそのまま網になっている。** 一時的な 404 で放送中の番組を
消してしまっても、その番組は notifybox に載り続けるので `warnNotifyboxResurrection` が鳴る。
**404 を消す側へ回すにあたって新しい見張りを足す必要が無かったのは、これがあるから。**
逆に言えば、**この罠を外すと 404 の誤削除が無言になる。** 消さないこと。
...
### 🔴 通信量の心配は決着済み。3回目を蒸し返さないこと

**「`on_air` の答えは記録しないので、疑いに残り続ける番組を毎周期聞き直すのでは」**
という筋は、**3回測って3回とも否定されている。**

| いつ | 何を測ったか | 結果 |
|---|---|---|
| 2026-07-29（項目AD） | notifybox とフォローAPI の**集合と並び** | **完全一致** |
| 2026-08-02（項目BI-3） | 正常時（60番組が両方に載る）の**詳細API 回数** | **0回**／notifybox が空の異常時のみ 20回 |
| 2026-08-17 | 実機で差集合を直接数えた | notifybox 9件 ⊇ フォローAPI 8件 → **差 0件** |

**両方に載っているのが正常なので、正常時は疑いが1件も立たない。** 問い合わせが飛ぶのは
「notifybox から消えた直後の番組」だけで、それは1回 `ended` を確認したら記録されて聞き直さない。

⛔ **`api.cas.nicovideo.jp/…/programs/lv…/statistics` への置き換えは却下。**
2026-08-17 実測で **78バイト**・`{"data":{"liveCycle":"on_air","viewers":0,"comments":0}}` と
`liveCycle` 入りであることは確認した（現行の詳細APIは約2KB）。
**が、正常時の呼び出しが 0回 なので減らす対象が無い。** 効果ゼロの置き換えのために、
「終了した番組にも 200 を返すか」という未測定のリスク（404 なら**終わった番組が永久に消えない**）を
背負う理由が無い。⚠️ **live2 の `/statistics` とは別物。** あちらに `liveCycle` は入っていない（項目CT）。
...
- **BF-2⑩／⑪** 404 は消す／全員404は消さない／2件なら全員404でも消す

🔴 **⑩⑪ を書く時は「生き残る番組を1つ残す」こと。** 疑いを全部消える形にすると
`combined.length === 0` の分岐（放送中0件の周期は既存DOMを維持する）に入り、
**実装を壊しても同じ結果になる＝空振り検査になる。** 2026-08-17 に実際この形で書いてしまい、
「全員404なら消さない」のガードを外しても鳴らなかった。
...
## ✅ CL. 新着番組がいきなり上位に入り、じわじわ落ちていた（2026-08-10・利用者報告）

「若い番組はいきなり上位に入ってきて、徐々に落ちていく」。**説明していた動きと逆だった。**

### 仕組み

推定同接は `到着レート × min(W, 経過分)`。初回は前回値が無いので到着レートを
`累計来場者 ÷ 経過分` で仮置きする。すると **経過分どうしが打ち消し合い、
推定同接 ＝ 累計来場者そのもの**になる。

🔴 **ニコ生の来場者は、入ってすぐ閉じた人も数える。**「まだ誰も帰っていない」という
`min` の仮定が、新着番組では成り立たない。だから実際の数倍で出る。

その後は実測の増分レートへ移っていくが、**均し（τ＝3分）が仮置きを少しずつしか手放さない**ので
落ち着くまで20分かかっていた。これが「じわじわ落ちる」の正体。

実コードで再現（立ち上がり5分で300人 → 以後 12人/分。真値は約120）:

```
経過分   改修前   改修後   真値
    5     300      300     約120   ← 材料が累計来場者しか無い。ここは変わらない
    7     257       84     約120
    9     222      108     約120
   11     185      120     約120   ← 落ち着いた
   25     121      120     約120   ← 改修前はここまでかかっていた
```

### 🔴 `min` のクランプは「若い番組を持ち上げる」ものではない

**抑えているほう。** 外すと5分時点で 300 → **600** になる（実測の再現で確認）。
2026-08-10 に一度「若い番組が徐々に上がってくる設計」と説明したが**誤り**。
係数が伸びるぶん到着レートが下がるので打ち消し合い、実際は上がらない。

### 直し方

**仮置きの値は、実測が1つ取れたら混ぜずに捨てる。** `nextViewerRate` の戻り値を
`{rate, seeded}` にし、`seeded` を storage に持ち回して次回に見る。
落ち着くまで 約20分 → 約6分。平均のずれ 50人 → 21人。

⚠️ **増分が 0 の周期では置き換えないこと。** 静かな番組が一発で 0 に落ち、順位が最下位へ
吹き飛ぶ。0 の時は従来どおり均し、仮置きの印も残して次の機会を待つ。
⚠️ 置き換えた後は普通の均しに戻す。実測どうしを1回で飛びつかせると別の跳ね方をする。
🔴 **印（`viewerRateSeeded`）を storage に保存し忘れないこと。** 落とすと毎回
「仮置きではない」と誤認して置き換えが二度と起きず、**計算式は正しいのに元の動きへ戻る。**

### τ を伸ばす案はこの件には逆効果

「変動が激しい」への一般解は τ（均しの時定数）を伸ばすことだが、**この症状に対しては逆。**
仮置きを手放すのが遅くなり、落ちるまでの時間が延びる。**症状ごとに効く定数が違う。**

### 残っていること

初回の1回（上の表の300）は変わらない。その時点の材料が累計来場者しか無いため。
潰すなら「新着は最初の1周期だけ順位を出さない」という別の判断が要る。**未着手。**

### 検証

`verify:loop` の項目CL（12件）。仮置きの判定・置き換え・増分0の据え置き・
クランプが残っていること・**通しで12分以内に真値の±5%へ入ること**（回帰の砦）・
storage が印を保存すること。空振り検査4通りとも狙った項目が鳴った。

---

## ✅ CM. 表示順序「おすすめ」（2026-08-10・利用者要望）

よく見る配信者ほど上に出す。**点数制**（開いた時+1／視聴6分ごと+1・1回の視聴で最大+10）。
数えるのは**配信者**。ニコ生は放送ごとに lv が変わるので、番組で数えても次の放送に効かない。

### 🔴 配点は2つの数で決まる。混ぜて考えないこと
...
**片方だけ動かすと両方が黙って変わる。** 検査 CM が積と上限の**両方**を見ている
（積だけ見ていた時期があり、6点→11点の変更が素通りした）。

```
 5分ごと・上限 5   25分で満点・最大 6点   初版
12分ごと・上限 5   60分で満点・最大 6点   2026-08-11（満点までを延ばした）
 6分ごと・上限10   60分で満点・最大11点   2026-08-12（満点の価値を上げた・現在）
```

🔴 **2026-08-12 の変更理由**（利用者報告）: 「開いただけで1点なのに比べて、長く見た番組が軽い」。
旧配点では **5回開いただけの配信者が1時間見た配信者に並んでいた**（6点対6点）。
満点までの60分は利用者が選んだ値なので据え置き、**刻みと上限を同時に半分／2倍**にして
積を保ったまま価値だけ上げた。刻みも縮めたのは途中まで見た場合も厚くするため
（30分視聴が 3点 → 6点。**上限だけ上げると長時間視聴しか救えない**）。

⚠️ **過去に貯めた点は昔の配点のまま。** 1回ごとの履歴を持っていないので移行できない。
結果として最近見た配信者が相対的に強くなる。承知のうえでの変更（リリース前で利用者は1人）。

🔴 **保存は `chrome.storage.local`。** `localStorage` はオリジンごとなので、
ニコ生と kick.com で履歴が分かれる。
🔴 **記録側とカード側でキーが一致しないと、数えても順位に出ない。**
しかも例外もログも出ず「全部0のまま並ぶ」だけ。実測で
`programProvider.id` と視聴ページの `/user/<id>` が一致することを確認して検査に固定した。

### 実機で見つけた3件

| 症状 | 原因 |
|---|---|
| 何回開いても増えない | 「そのタブで既に見た相手」を弾いていた。**同じ配信者を続けて開いた時**まで弾かれる |
| 別タブの分が反映されない | 履歴を起動時に1回しか読まない。`storage.onChanged` で追従させた |
| **点数が減る** | メモリ上の表を丸ごと保存していた。**別ページの加点を消す**（lost update） |

🔴 3つ目は `patchProgramThumbnail` が同じ理由で同じ形（読み直して1件だけ足す）になっている。
**サイドバーはニコ生と kick.com の両方に居るので、書き手は常に複数**という前提で書くこと。
⚠️ 二重計上を防ぐのは**リロード判定だけでよい**（`performance` の navigation type）。
タブ単位で弾くと取りこぼす。

---

## ⛔ CN. 「人気順の基準」の目盛りを校正した（2026-08-10に2度・08-11に1度）→ **2026-08-12 に設定ごと廃止**（項目CU）

以下は当時の記録。**目盛りもスライダーも今は存在しない。** W は 17分 固定。


利用者が他サイトの推定同接と実機で比較して判明。

```
① [3, 5, 8, 10, 14, 20, 30, 45]      既定10  … 全体に短すぎた
② [10, 17, 27, 40, 58, 80, 110, 150] 既定40  … 今度は幅が広すぎた
③ [11, 13, 15, 17, 19, 22, 25, 29]   既定17  … 17分が正解。刻みを細かく
④ [8, 10, 12, 14, 17, 20, 24, 29, 35] 既定17 … ③は狭すぎ、中央も無かった（現在）
```

🔴 **目盛りは奇数個にすること**（2026-08-11・利用者要望）。③は8段で、**ちょうど真ん中の
目盛りが存在しなかった**（既定17は左から4番目）。既定値はつまみの中央に置く。
④は9段で中央（添字4）が17。幅は 2.64倍 → **4.38倍**、対数中心 16.7分。

⚠️ 刻みの比は 1.17〜1.25 の等比。**等間隔にしないこと**（W の効き方が対数的で、
等間隔にするとスライダーの右半分が死ぬ）。

🔴 **目盛りを差し替えたら移行の印の版も上げること**（`dwellScaleV3` → `dwellScaleV4`）。
上げないと、既に移行済みの利用者は**新しい目盛りに載っていない値のまま**になり、
つまみの位置と実際に効いている値がずれる。⚠️ `optionKeys` にも同じ文字列が要る（項目CP）。
④での寄せ先: `11→10 / 13→12 / 15→14 / 17→17 / 19→20 / 22→20 / 25→24 / 29→29`。

✅ **17分 は再確認済み（2026-08-12・利用者）。** 2026-08-11 に推定同接の式を作り直した
（項目CQ）ため、17分は**旧実装の偏りを平均で埋め合わせた値**で持ち越せないと見ていたが、
新しい式でも**スライダー中央（17分）でちょうどよい**と確認された。校正は完了。

⚠️ **ここは合わせ込みの目盛り。実機比較の裏付け無しに動かさないこと。**

### 検査（CN群）

- 奇数個か／ちょうど真ん中が既定値か（**両方見る**。片方だけだと中央が消えても気付けない）
- 既定値が目盛りに実在するか／昇順で重複が無いか／比が等比に近いか
- 移行の印が `optionKeys` に載っているか。⚠️ **印の名前は実装から読む**
  （直書きすると、目盛りを変えるたびに検査側の書き換えが要る）

⚠️ **保存値は「分」で、最寄りの目盛りに吸着する。** 目盛りを変えると全員の値が動く。
移行を書かないと、真ん中に居た人が端へ飛ぶ。
🔴 **範囲外の古い値は既定へ寄せる。** 端へ丸めると、利用者が意図しない極端な設定のまま使い続ける。
⚠️ **1回だけ動かす印を残すこと。** 印が無いと、利用者が端を選ぶたびに既定へ戻され続ける。

---

## ✅ CO. 推定同接の作り替え（2026-08-10・利用者報告3件がすべて同じ原因）

```
旧  直近2分の到着レート × 滞在時間        ← 2分の観測を20倍に引き伸ばす
新  Σ（各区間に入った人数 × exp(-経過 / 滞在時間)）
```

### 引き伸ばしが生む3つの破綻（すべて実測で再現）

| 症状 | 旧 | 新 |
|---|---|---|
| 来場70人・直近2分で+5人 | **8人**（山の前に入った人が消える） | 70人 |
| 累計200人・一時的に集中 | **720人**（累計を超える） | 34人 |
| 毎分12人で安定・直近2分だけ0 | **244人**（半減） | 474人 |

🔴 **累計来場者を超えるのはあり得ない。** 上限として入れた（恣意的な係数ではなく事実）。
⚠️ Kick は実測値で来場者の概念が無いので、上限より**前に**返す。

### 窓をきっぱり切らないこと

最初は「直近 W 分に入った人数」にしたが、**直近 W 分に誰も入らない番組が 0 になった**
（画面に「—」と出て、しばらくすると戻る）。実際には W より前に入った人も残っている。

滞在時間を**平均 W の指数分布**とみなし、古い到着ほど軽く数える形にした。
**定常状態では レート×W に収束する**ので目盛りの校正はそのまま効き、
0 に落ちることも窓の縁で段差ができることも無い。

### 材料

番組ごとに来場者の履歴（時刻と累計の組）を `programInfos` に持つ。
⚠️ 間引き（2分）・古い点の破棄（170分）・件数の歯止め（90）が無いと保存が膨らむ。
🔴 **保持は目盛りの最大より長くすること**（検査で固定）。
🔴 **渡された info にも書き戻すこと。** 保存用のコピーにだけ書くと、画面はいつまでも従来式で出る。

### 残っている偏り

**規模が大きい番組ほど低めに出る。** 大きい配信は視聴者が腰を据え、小さい配信は
「覗いて数秒で去る」割合が高い＝**実際の滞在時間が規模で違う**のに、全番組へ同じ値を当てている。
直すなら規模に応じて滞在時間を伸ばす形になるが、**係数を2つ増やす**話なので
実測を集めてから。**未着手。**

### 404 は警告しない

番組詳細APIを叩く3箇所（終了確認・サムネ追撃・穴埋め）はどれも「まだ在るか」を聞く用途。
番組が終われば消えるので **404 は予定どおりの答え**。警告を出すと正常運転でコンソールが埋まり、
本物の異常が埋もれる。他の状態コードは今までどおり警告する。

---

## ✅ CP. 設定を保存すると視聴点数が巻き戻る（2026-08-11・利用者報告「おすすめ順の点数が急に0に戻る」）

**タブは1つでよい。** 項目CM で直したのは「タブどうしの奪い合い」だったが、これは別の書き手。

```
getOptions  = { ...既定, ...chrome.storage.local.get() }  ← キー未指定＝storage 全体が返る
saveOptions = その丸ごとを書き戻す                         ← 視聴履歴まで一緒に書き戻る
```

options が持っているのは**ページを開いた瞬間のスナップショット**。設定を1つ変えるだけで、
それ以降に貯まった視聴点数が読み込み時点まで巻き戻る。初見の配信者なら **0pt に戻る**。

🔴 **人気順の基準スライダーは指を離した 400ms 後に保存する。** 実機テストでスライダーを
   触るたびに巻き戻っていた。**エラーもログも出ない。**

巻き添えになるのは視聴履歴だけではない。Kick のアイコンキャッシュ・自動移動の印も含め、
**chrome.storage.local にあるもの全部**がそのタブを開いた時点の姿に戻る。

### 直し方

constants.js の `optionKeys` に「設定であるキー」を並べ、storage.js の `pickOptionKeys()` を
**storage へ書く手前の唯一の関所**にした。`getOptions` の書き戻しと `saveOptions` の両方が通る。

⚠️ **設定を足したら `optionKeys` にも足すこと。** 落とすと「その設定だけ無言で保存されない」。
⚠️ 値が無いキーは書かない。undefined を渡すと storage に null が入り、次回の既定値マージを潰す。

### 検査（CP群・`npm run verify:loop`）

正規表現ではなく**実際に書き戻して確かめる**。chrome.storage.local を本物と同じ
「キー単位マージ」で作り、`getOptions → recordWatch → saveOptions` を実コードで順に呼ぶ。
**壊して鳴ることを確認済み**（`toSave` を `{ ...options }` へ戻すと3項目が NG）。

- main.js / kickPage.js の defaultOptions が全部 `optionKeys` に載っているか（足し忘れ検知）
- 関所を迂回した `chrome.storage.local.set` が無いか
- 設定そのものは保存できているか（巻き込み防止で保存を殺していないか）

---

## ✅ CQ. 推定同接が「人数」ではなく「観測時間」を測っていた（2026-08-11・利用者報告2件が同じ原因）

利用者報告は別々の2件だった:

1. 「**—がまだ出る。通信が安定しない環境で特に**」
2. 「スライダー真ん中で他サイトに近くはなったが、**近ければ良いというわけではない**」

**どちらも同じ欠陥。** 実コードを回して測った（毎分5人・W=17 → 真値85）:

| サイドバーが見ていた時間 | 2分 | 5分 | 17分 | 30分 | 60分 | 120分 |
|---|---|---|---|---|---|---|
| 旧 | 16 | 48 | 46 | 63 | 75 | 77 |
| 新 | **85** | **85** | **85** | **85** | **85** | **85** |

🔴 **旧実装の表示は「サイドバーがその番組を何分見ているか」で5倍変わっていた。**
   同じ人数の番組が、カードによって 16 とも 77 とも出る。

### なぜそうなったか

来場者の履歴に入っているのは**サイドバーが見ていた間の到着だけ**で、それ以前の到着は1件も
入っていない。旧実装はそれを `span / W` の線形割り戻しで埋め合わせていたが、**減衰の形と違う。**
減衰は `exp(-経過/W)` なので、覆えている割合は `span/W` ではなく `1 − exp(-span/W)`。
span = W ちょうどの時点で真値の 63% しか覆えていないのに、コードは 100% 覆えたつもりでいた。

**「—」はこの欠陥の極端な場合。** 観測中に誰も来なければ和が 0 になり、
**累計500人の番組でも 0** と出る（画面では「—」）。通信が不安定だと観測が短く途切れるので当たりやすい。

🔴 **W ではこの偏りを直せない。** W は全カードへ同じ倍率を掛けるだけで、偏りはカードごとに違う。
   目盛りを合わせても、合わない相手が必ず残る。**「他サイトに近い」を校正の根拠にしないこと。**

### 新しい式（場合分けを持たない）

```
推定同接 = Σ（その区間に来た人数 × exp(-経過 / W)）
         + 観測前のぶん: レート × W × (e^(-a/W) − e^(-b/W))
           レート = 観測開始時の累計 ÷ そこまでの放送時間
           a = 観測開始からの経過 / b = 放送開始からの経過
```

🔴 **場合分けを戻さないこと。** 旧実装の「放送が W より若ければ累計そのもの」という枝は、
   経過が W をまたぐ瞬間に段差を作っていた（実測 300 → 186）。1本の式なら跳ねない。
⚠️ 履歴が1点も無い周期もこの式で通る（観測前の項だけが残る）。専用の代替計算は要らない。
   これに伴い `viewerRate` / `nextViewerRate` は**推定の経路から外れた**（覗き窓として残置・消してよい候補）。

### 一緒に直した2つ

**(1) 履歴の末尾は「可動点」にする。** 旧実装は間引きの時 `[前の時刻, 新しい値]` と書いていた。
「その時刻に累計はこれだった」が嘘になり、**次の点との差分＝その区間に来た人数が足りなくなる。**
いちばん重みの大きい直近の区間で起きるので、推定は常に1割ほど低く出た（真値85に対し 77 で頭打ち）。
⚠️ 確定させる判定は**可動点の今の時刻**で行う。`now` で判定すると確定点どうしが間隔より詰まり、
   170分ぶんが上限（90点）に収まらず古い方から捨てられる。

**(2) 取得が止まっている間は時計を進めない**（`viewerSampleStaleGraceMs` = 4分）。
この式は `now` が進むだけで全部の項が減る。通信が切れている間じゅう推定値が独りでに溶けていた
（実測: 85 → 24 → 1 →「—」）。番組は放送中で人も居るのに、こちらの通信事情だけで数字が消える。
⚠️ 凍らせるのは**取得できていない時だけ**。取得が成功していて誰も来ない番組は最新サンプルの
   時刻が進むので、ちゃんと減っていく（実測 138 → 44）。

### 症状シナリオでの前後（実コード・`scripts` の観測点で測定）

| シナリオ | 旧「—」回数 | 新「—」回数 | 新の値の動き |
|---|---|---|---|
| 安定・毎分5人 | 0/40 | 0/40 | 83〜85 で安定（旧は 16〜102） |
| 静か（累計500・新規ゼロ） | **36/40** | 0/40 | 138 → 44 と減衰 |
| 通信不安定（2/3失敗） | 0/40 | 0/40 | 78〜85（旧は 26〜91） |
| 切断しっぱなし | 2/40 | 0/40 | 猶予後に凍結 |
| 若い番組 | 0/30 | 0/30 | 段差なし（旧は 340 → 202） |

### 残っていること

**数字が出ない場合はまだ1つある。** 来場者数そのものが未取得（`viewers: 0`）の間は「—」になる。
これは「推定できない」ではなく「**まだ何も分かっていない**」なので、数字を作らずそのままにしてある。
⛔ **詳細APIで埋める案は 2026-08-12 に実測して却下した**（項目CR）。詳細APIのほうが 91〜273秒 遅い。

✅ **目盛り（W）の校正し直しは完了**（2026-08-12・利用者が新しい式で再確認）。
新しい式でも**スライダー中央（17分）でちょうどよい**。項目CN を参照。

**規模による滞在時間の違いは未着手のまま**（項目CO の「残っている偏り」）。今回の修正とは独立。

### 検査（CQ群・`npm run verify:loop`）

実コード（`upsertProgramInfos` → `estimateConcurrentViewers`）を周期ごとに回して数字で確かめる。
**いちばん大事なのは「観測2分でも120分でも同じ値」**。ここが崩れたら表示は観測時間を測っている。

- 定常番組が「到着レート×W」に一致するか（真値べた書き。実装から作らない）
- 新規が来ない番組が 0 にならないか／それでも減っていくか
- 1周期の変化が 15% 未満か（W 境界の段差検知）
- 累計来場者を超えないか
- 取得できない間に減り続けないか
- 確定点の間隔が詰まっていないか／末尾が可動点になっているか
- Kick の実測値を推定に巻き込んでいないか
- 壊れた入力で NaN を返さないか

---

## ✅ CM-2. テスト用の表示（2026-08-10 → 2026-08-13 に決着）

実機で数字を確かめるために出していたもの。**全部片付いた。**

| 覗き窓 | 場所 | 決着 |
|---|---|---|
| 視聴点数（`7pt`） | 配信者名の行の右端 | ✅ **1.20.4 で撤去**（下記） |
| 同時視聴者数 | サムネ左上 | ✅ β版の設定「同時視聴者数」になった（項目CR） |
| `data-total` / `data-comment-weight` / `data-comment-ratio` | カードのDOM属性 | ✅ **1.20.5 で計算ごと撤去**（下記） |
| `momentum` / `viewerRate` | storage のレコード | ✅ **1.20.5 で計算ごと撤去**（下記） |

⚠️ サムネ左上の表示は `pointer-events: none` 必須。サムネは `<a>` の中なので覆うと押せなくなる。

### 視聴点数（`7pt`）の撤去（1.20.4）

消したのは3箇所:

- `makeProgramElement` のバッジ生成（`.watch_count_badge` の span）
- `applyRankAttributes` のバッジへの書き込み2行
- `main.css` の `.watch_count_badge`

🔴 **`data-watch-count` は残す。** よく見る順の並べ替えがこの属性を読む
（`utils/programOrder.js`・書き手は `applyRankAttributes` だけ）。
**表示だけを消して、値は並べ替え専用として残す**のが今の形。

⚠️ **そもそも画面に出してはいけない言葉だった。** 「視聴ポイント」は自作用語で、
画面では「視聴履歴」と呼ぶ（項目CW）。テスト表示が自作用語のまま出ていた。

検査は CM に2件（`npm run verify:loop`）。**ソースと実物の両方を見る。**

- ソース: `watch_count_badge` が `sidebar.js` にも `main.css` にも無い／`+ 'pt'` を書いていない
- 実物: 生成したカードに `.watch_count_badge` が無い（空振り防止に `.provider_name` が引けることを先に見る）

⚠️ CSS だけ消し忘れても画面は変わらないので**気付けない**。3箇所まとめて見ること。

### DevTools 用の覗き窓と、その裏の計算の撤去（1.20.5）

画面には出ないが、**毎周期すべての番組ぶん計算して書き続けていた**もの。
読み手を辿ったら、どれも**自分自身しか読んでいない閉じた輪**だった。

| 消したもの | 何だったか | いつ順位から外れたか |
|---|---|---|
| `data-total` / `data-comment-weight` / `data-comment-ratio` | カードのDOM属性（DevTools で覗く用） | 2026-08-04 |
| `nextMomentum` / `initialMomentum` | 勢い。storage の `momentum` へ保存していた | 2026-08-04 |
| `commentWeight` / `commentRatio` / `totalEngagement` | 弾幕補正。勢いの中でしか使わない | 同上 |
| `nextViewerRate` / `initialViewerRate` | 到着レート。storage の `viewerRate` へ保存していた | 2026-08-11 |
| `momentumTauMs` ほか定数5つ | 上記の係数 | 同上 |

🔴 **残すのは `estimateConcurrentViewers` と `estimateFromArrivals` の2つだけ。**
推定同接の材料は来場者の履歴（`viewerSamples`）だけで、上のどれも使っていない。

⚠️ `momentumTauMs` は Kick の同接の平滑化（`kickConcurrentTauMs`）が参照していたので、
**値（8分）と理由をそちらへ引き取ってから**消した。単に消すと Kick の平滑化が壊れる。

⚠️ ファイル名は `momentum.js` のまま。改名は import 4箇所と doc に波及するので見送った。

#### 🔴 検査は「消した」ではなく「入り込まない」を見る形へ

弾幕の検査（BE）は「弾幕番組より本物が上に来る」を見ていた。補正が消えた今も**通る**が、
それは**コメントを完全に無視していても通る**からで、意味が変わっている。
コメントが順位へ混ざり始めても気付けないので、**同じ入力でコメントだけ変えて
推定同接が1ミリも動かないこと**を見る形へ差し替えた。

⚠️ **描画経路を2回走らせて比べないこと**（2026-08-13 に踏んだ）。推定同接は `Date.now()` の
関数なので、2回目は実時計が進んだぶん末尾の桁が違う（実測 39.7287 対 39.7287 で、
目には同じでも `1e-9` では一致しない）。かといって時計を止めると `viewerSamples` の時刻まで
潰れて推定が成立しない。**同じ `now` を渡せる式のほうで見る**のが正しい切り分け。

`estimateFromArrivals` の `cum` にコメントを足し戻すと、この検査が落ちる（確認済み）。

#### ⚠️ 検査が「名前の出現数」を数えていた

`data-watch-count` の書き手が1つであることを、**生の文字列の出現数**で見ていた。
説明コメントで属性名に触れただけで落ちた（2026-08-13）。
`setAttribute('data-watch-count'` の数を数える形へ直した。**数えるのは書き手であって名前ではない。**

---

## ✅ CR. 同時視聴者数の表示（β版）と既定値の一元化（2026-08-12）

サムネ左上の同接を、テスト用の出しっぱなしから**設定でON/OFFできるβ版**にした。
**既定は OFF**（動くサムネβ版と同じ扱い。更新しただけで既存の見た目が変わらない）。

### 出し分けの作り

判定と印付けは `src/ui/viewerCount.js` **1箇所**（`ui/placement.js` と同じ形）。
印は `<html>` の `nns-show-viewers`。⚠️ **`<body>` ではない**（kick.com のサイドバーは
`<html>` 直下に居るので body の印は届かない）。

🔴 **印の向きは「出す時に付ける」。** 逆（隠す時に付ける）にすると、この配線が動かなかった時に
**既定OFFのはずの数字が出る**。CSS も `.viewer_overlay` の既定を `display:none` にしてあり、
`html.nns-show-viewers` の時だけ `block` になる。壊れ方が「出ない」側へ倒れる。

🔴 **カードは作り直さない。** バッジは常に生成して `applyRankAttributes` が中身を書き、
出し分けは CSS だけがやる。切り替えが即時に効き、描画中に順位がずれない。
**描画側（sidebar.js）は設定を見ない**（見ると判定が2箇所になる）。

### 「計算中」は放送開始から数分出る（仕様として残す・利用者判断）

⚠️ **画面の文言は 2026-08-12 に「—」から「計算中」へ変えた**（利用者指定）。
   0 になるのは「推定できない」ではなく「ニコ生側の集計がまだ始まっていない」＝待てば出る。
   「—」は情報が無いことしか伝えないが、「計算中」なら待てばよいと分かる。
   ⚠️ **これより前の記録に出てくる「—」は当時の表示**（書き換えていない）。


`viewers`（累計来場者）が 0 の間は推定同接も 0 になり「計算中」と出る。**ニコ生側の集計が
始まっていないため**で、こちらの取得の遅さではない。2026-08-12 の実測:

| | 0 を抜けるまでの放送経過 |
|---|---|
| 一覧API（フォローAPIと同一スキーマの公開 recent 版・7件） | **53〜224秒**（中央値 107秒） |
| 番組詳細API（`api.cas.nicovideo.jp/.../programs/lv…`・6件） | **315〜332秒**（全件 5分15秒〜5分32秒） |

⛔ **詳細APIで埋めるのは却下**（項目CQ の「残っていること」に挙げていた案）。
詳細APIの `viewers` は**一覧APIより 91〜273秒 遅く**、一覧が既に人数を返している間ずっと 0 を返す
（実測: 経過108秒で 一覧=4／詳細=0、経過196秒で 一覧=66／詳細=0）。0 で上書きしない作りにすると
結局一覧APIを待つのと同じになる。**改善にならないので、この案は閉じた。**
なお 0 を抜けた後の値は詳細APIのほうが少し新しい（40 に対し 44／89 に対し 95）が、
「—」を早く消す目的には効かない。

### 🔴 既定値を各ページへ直書きしていた（同時に見つけた欠陥）

`main.js` / `kickPage.js` の `defaultOptions` が `dwellMinutes: 10` を直書きしており、
`constants.js` の `defaultDwellMinutes` を 17 にした後も取り残されていた。
**＝新規利用者に実際に効く W は 17 ではなく 10**（目盛り①の既定10 の名残）。

無言で残った理由: 10 は今の目盛り `[8,10,12,14,17,20,24,29,35]` の**中に在る**ので、
項目CN の移行が「範囲内＝最寄りへ」で 10 のまま通す。目盛りの中央判定（CN）は
`constants.js` どうしを突き合わせているだけなので、ここを見ていなかった。

対応: 両ページとも `defaultDwellMinutes` / `defaultCardSize` / `defaultShowViewerCount` /
`SIDEBAR_PLACEMENT_DEFAULT` を**読む**形にした。⚠️ **既定値を直書きしないこと。**

### 検査（CR群）

- 既定は OFF ／ 知らない値は OFF へ倒れる ／ **出す時だけ印が付く**（実際に呼んで確かめる。
  偽 DOM の `classList.toggle` は第2引数を無視しない本物と同じ形にすること）
- 印は `<html>`（`document.body` を触っていない）
- 両ページが `applyShowViewerCount` を呼ぶ／**初期化と設定変更の両方**で呼ぶ（2回以上）
- 設定 UI（β版の印・OFF/ON・保存・復元）／**保存の必須ガードに入れていない**
- CSS の既定が非表示・印が付いた時だけ表示／描画側が設定を見ていない
- 🔴 **constants の `default*` と両ページの `defaultOptions` が食い違わない**（総当たり）
- 🔴 **`getOptions` を通した結果の W が目盛りのちょうど中央**（直書きが消えても、移行が
  寄せ直せば元の木阿弥。**実際に効く値**で見る）

---

## ✅ CS. 自動移動の直後に、終了した番組のカードが残る（2026-08-12・利用者報告）

**症状**: 見ていた番組が終わって自動移動したあと、その終わった番組がリストに残っていることがある。
**毎回ではない。**

### 原因: 「知っているのに聞き直していた」

終了検知（項目BF-2）は**一覧APIに載っていない番組を詳細APIに聞く**形なので、
**一覧APIがまだ載せている間は疑いにすらならない。** そして一覧APIには遅れがある。

2026-08-12 の実測（公開の recent 版・15秒ごと・20分）:

| | |
|---|---|
| 番組が `ended` になった時点で一覧APIは | **まだ載せている**（捕まえた2件とも） |
| 一覧APIから消えるまで | **0〜30秒**（15秒刻みで測って2件とも1周ぶん） |
| 自動移動が一覧を取りに行くのは | 終了 → 10秒カウント → 遷移 → 300ms ＝ **約15〜20秒後** |

**窓が重なっている。** だから出たり出なかったりする。残った場合は次の定期更新（既定120秒）で消える。

🔴 **本質は「自動移動はその番組が終わったのを自分で見ている」のに、持ち越していなかったこと。**
それが移動のきっかけなのに、ページを移ると記憶が消え、飛び先で一から聞き直していた。

### 直し方

移動する直前に「この番組は終わった」を `chrome.storage.local` へ置き、飛び先が読む。
ニコ生側の反映が何秒かかろうと関係なくなる。

- 印は `autoNextEnded` = `{ id: 'lv…', at }`。**`autoNextHop` に相乗りさせないこと**
  （あちらは `consumeAutoNextHopMark` が視聴回数を数えるために**早い段階で1回で使い切って消す**）
- `markAutoNextHop(飛び先, 離れる番組)` の**1回の呼び出しで両方書く**。呼び出し側は2箇所
  （カウントダウン満了・サムネクリック）あり、別関数にすると片方だけ忘れる
- 読むのは**取得を始める前**。ニコ生は約束（`_endedSeeded`）を `_dropEndedPrograms` の頭で
  await、kick.com は `startTimer()` より前で await
- **ニコ生だけを対象にする**（`nico:lv…` の形だけ拾う）。Kick は「今回のリストに居ない＝終了」で
  判定でき、詳細APIに聞く仕組み自体が無い（項目BX）

### 🔴 `_endedConfirmed` に混ぜてはいけない（実装して検査で気付いた）

`_endedConfirmed` には**鳴る罠**がある ――「消した番組が notifybox に戻ってきたら印を落として警告」。
詳細APIの答えと notifybox の食い違いを捕まえるための仕掛けで、これは残すべきもの。

ところが**この印が効いてほしいのは、まさに一覧APIも notifybox もまだ返している窓**である。
混ぜると**置いた瞬間に罠が落とし、しかも誤警告まで出た**（検査 CS が実際にこれで落ちた）。

そこで別枠 `_endedByAutoNext` にした。鳴る罠の対象外・**期限つき**（`endedByAutoNextValidMs` = 3分）。
⚠️ **罠の外に置く以上、取り違えた時に自分で戻れる道が要る。** それが期限。
⚠️ **期限は両ページで同じ定数を読むこと**（片方に数値を直書きするとずれても気付かない）。

### 検査（CS群）

- 印の読み書き（別キー・1回で消える・期限切れは無視・壊れた値では消さない・Kick では置かない）
- 🔴 **一覧も notifybox も返し続けている状態で、離れた番組が出ないこと**（実コードを回す）
- そのために詳細APIを叩いていないこと／印が無ければ1件も消えないこと
- `markAutoNextHop` の呼び出しが2箇所とも引数2つであること
- 種蒔きを await していること／種蒔きが throw しないこと
- kick.com も同じ印を読み、**取得を始める前に**読むこと

⚠️ **検査の書き方で3回踏んだ。**
1. `markAutoNextHop\([^)]*,[^)]*\)` … 実引数が `watchTargetIdOf(nextHref)` と入れ子なので
   どう書いても嘘になる。**括弧の深さを数える。**
2. `indexOf('startTimer()')` … 関数の**定義**が先に出るので常に「呼び出しより前」で合格した。
   init の中だけで比べる。
3. `bodyOf(src, '_seedEndedFromAutoNext()')` … **コンストラクタの呼び出し側**に先に当たり、
   別の波括弧を本体として取り出していた。定義（`async …`）で探す。

🔴 **さらに、storage のスタブを即座に解決する形にしていたため、`await` を消しても
   振る舞いの検査が通ってしまった。** 本物は必ず1往復するので、スタブも次のタイマーまで
   返らない形にした。これで順序を壊すと振る舞い側も鳴る。

---

## ✅ CT. 来場者数を live2 から取る（2026-08-12・利用者要望）

発端は「番組ページでは開始直後から数値がある。もっと早く受け取れるAPIがあるはず」。

### 見つけたもの

`https://live2.nicovideo.jp/watch/{lv}/statistics`
→ `{"data":{"watchCount":3552,"commentCount":995}}` **86バイト・約100ms**。

利用者の実機で15秒ごとに一覧APIと並べた実測:
...
🔴 **遅れよりも「粒度」のほうが効く。** 一覧APIは**約60秒に1回しか動かない**（階段状）。
推定同接は来場者の**増分**から計算しているので、60秒粒度だと増分ゼロの周期が大量に出る
（2026-07-31 実測: 30秒窓の79%がゼロ。時定数8分で均しているのはこのため）。
live2 は15秒ごとに動くので、材料そのものが細かくなる。ついでに新着の「—」も早く消える。

⚠️ **`liveCycle` は入っていない。** 終了確認（項目BF-2）の置き換えには使えない。

### 却下した案（実測で決着・項目CR にも記録）

⛔ **番組詳細API で埋める**。`viewers` が 0 を抜けるのは放送開始から **315〜332秒**で、
一覧API（53〜224秒）より **91〜273秒 遅い**。改善にならない。

### 作り

**混ぜ方は `src/services/liveStatistics.js` の `applyLiveStatistics` 1箇所。取り方だけが違う。**

| | 取り方 | ホスト権限 |
|---|---|---|
| ニコ生ページ | 直接 fetch | **要らない**（live2 が `live.nicovideo.jp` へ CORS を開いている） |
| kick.com ページ | SW 経由（`nico:statistics`） | 要る（`live2.nicovideo.jp/*`・optional） |

🔴 **写像（`mapApiProgramToInfo`）より前に、生データへ入れること。** 後だと programInfo に届かない。
ここで直しておけば、その先（推定同接・順位・表示）は何も変えずに済む。

🔴 **小さくしない（`Math.max` で入れる）。** 取れた番組と取れない番組が混ざる周期に値が
行ったり来たりすると、`appendViewerSample` / `nextViewerRate` が「誰も来ていない」と読む。

⚠️ **取れなくても描画は止めない。** 未ログイン（401）・権限なし・SW 停止のいずれでも、
一覧APIの値のまま従来どおり出る。

### 🔴 権限の罠（いちばん危ない）

`hasKickPermission()` は `chrome.permissions.contains(KICK_PERMISSIONS)` で**丸ごと**照合する。
ここに origin を1つ足すと、**既に Kick を許可している利用者が全員 false になり、
Kick 連携そのものが止まる。**
...
| 集合 | 用途 | live2 |
|---|---|---|
| `KICK_PERMISSIONS` | **確認（contains）と削除（remove）** | 入れない |
| `KICK_REQUEST_PERMISSIONS` | **要求（request）だけ** | 入れる |

live2 の許可は `hasLive2Permission()` で**別に**確かめる。無ければ中継が
`no-permission` を返すだけで、kick.com のリストは従来どおり出る。
⚠️ 既に Kick を有効にしている利用者は、**一度 OFF→ON しないと** live2 の許可が付かない
（付かなくても壊れない）。

### バンドル外との二重管理

`static/sw.js` は `import` が書けない（コピーされるだけ）。URL と同時数がこちらと重複する。
検査 CT が `constants.js` と `sw.js` を突き合わせている。**片方だけ変えると落ちる。**

### 検査（CT群30項目）

混ぜ方（上書き・小さくしない・取れない時に壊さない4通り・壊れた応答を採らない・
lv だけ問い合わせる）／両ページが同じ関数を通ること・写像より前であること／
constants と sw.js の突き合わせ／**権限の集合が確認用と要求用で分かれていること**／
中継が lv だけ受け取ること／manifest が optional のままであること。

⚠️ **検査でまた1つ踏んだ。** `/const KICK_PERMISSIONS = \{[\s\S]*?live2[\s\S]*?\n\}/` は
`[\s\S]*?` が**閉じ括弧を越えて**後続の live2 まで届き、入っていないのに入っている判定になった。
波括弧を数えて中身だけを取り出す形へ直した（項目CS と同じ型を3回踏んでいる）。

---

## ✅ CU. 「人気順の基準」を廃止／「おすすめ」を「よく見る順」へ（2026-08-12・利用者判断）

### 廃止の理由: 同接を画面に出した時点で筋が通らなくなった

同時に成り立たない2つを兼ねていた。

- 表示している数字が**実際の同接の推定**なら、正解は1つで、好みで動かすものではない
- 好みで動かす**つまみ**なら、それを「同時視聴者数」として画面に出してはいけない

β版として数字を出すと決めた（項目CR）以上、前者を選ぶのが一貫する。**W は固定にした。**
値は 17分＝旧スライダーのちょうど中央で、利用者が実機比較で「ちょうどよい」と確認した値（項目CN）。

⚠️ **失ったもの: ニコ生と Kick の釣り合いを調整する手段。** W はニコ生側だけに効くので、
統合表示で「ニコ生が上に来すぎる／来なさすぎる」と感じた時の唯一のつまみだった。
Kick は β版・任意なので許容した。**必要になったら、同接の表示とは別の名前・別の説明で作り直すこと。**

### 🔴 設定を消すだけでは足りない。読む経路も消す

消したもの:

| 場所 | 消したもの |
|---|---|
| `render/sidebar.js` | スライダーの HTML ／ `setDwellMinutes` ／ 可変の `dwellMinutes` |
| `handlers/optionsHandler.js` | `setupDwellSlider` ／ `onDwellChanged` 引数 ／ `syncActiveOnlySections` |
| `main.js` / `kickPage.js` | `defaultOptions.dwellMinutes` ／ `changes.dwellMinutes` ／ 呼び出し |
| `config/constants.js` | `dwellMinutesScale` ／ `optionKeys` から `dwellMinutes` と `dwellScaleV4` |
| `services/storage.js` | 目盛り校正の移行（`DWELL_SCALE_MIGRATED_KEY`）まるごと |

🔴 **保存値を読む経路を残すと、古い値を持っている環境だけ違う数字で動く。**
`calculateActivePoint` は `defaultDwellMinutes` を直接使う形にしてある。
前例がある ―― 項目CR で直したとおり、各ページが `dwellMinutes: 10` を直書きしていて
**新規利用者に効く W が 17 ではなく 10 だった。** 同じ形を作らない。

⚠️ **既に保存されている `dwellMinutes` / `dwellScaleV4` は消さない。** 読まなくなるだけで無害。
関所（`pickOptionKeys`）が一覧に無いキーを書かないので、そのまま放置される。
検査 CR が「古い残骸があっても効く W は変わらない」ことと「残骸を勝手に消さない」ことの両方を見る。

### 「おすすめ」→「よく見る順」

言葉が実態と逆だった。「おすすめ」は普通「まだ知らないもので好みに合いそうなもの」を指すが、
この機能は**既によく見ている配信者を上に出す**もので、新しい出会いは起きない。
他の2つが「新着順」「人気順」なのに1つだけ順序でない、という座りの悪さもあった。

🔴 **変えたのはラベルだけ。保存値は `recommend` のまま。**
値を変えると、保存済みの設定に対応するラジオが無くなり**その利用者は設定を一切保存できなくなる**
（項目BQ と同じ罠）。検査 CM が「値は recommend」「ラベルはよく見る順」「おすすめが残っていない」
の3点を同時に見ている。

### 検査（CU群）

- W は固定値を直接使い、可変の状態を持たない（`setDwellMinutes` が存在しない）
- `optionKeys` に `dwellMinutes` / `dwellScale*` が無い
- 設定 UI から消えている（`name="dwellMinutes"` も `opt-active-only` も無い）
- 3ファイルに差し替えの配線が残っていない／目盛りと移行が消えている（死にコードを残さない）

⚠️ **バックティックの罠をまた踏んだ。** 廃止の経緯を `optionHtml` の中の HTML コメントに書く際、
説明文に `` `defaultDwellMinutes` `` とバッククォートを使い、**テンプレートリテラルがそこで切れて
構文エラーになった**（doc/12 の冒頭に同じ注意書きがあるのに踏んだ）。あの中では記号を使わない。

---

## ✅ CV. 汚染画像を動くサムネへ渡していた（2026-08-12・利用者報告）

**症状**: コンソールに `⚠️ 動くサムネ: CORS汚染で解析不可` が出て、**そのページでは以後コマ化が止まる。**

### 原因: `feeding`（ONか）と「その読み方が安全か」を混ぜていた

kick.com では画像の配信元がどこも ACAO を返さないので、SW に取ってもらって data URL で受ける
（項目CD／`imageProxy`）。その中継が失敗した時に**素のURLで読み直して表示だけ守る**道がある。

```js
// 修正前
imageProxy.fetch(url).then(
    (dataUrl) => { pre.src = dataUrl || url },   // ← 中継が空なら素のURL
    ()        => { pre.src = url },              // ← 失敗しても素のURL
)
...
if (feeding) { ...ingest(card.id, pre)... }      // ← feeding は「機能がONか」でしかない
```

素のURLで読んだ画像には `crossOrigin` が付いていない＝**汚染画像**。それを②へ渡すと
`getImageData` が例外になる。

🔴 **1回で終わらない。** `sigCanvas` は使い回しで、**一度でも汚染画像を描くとその canvas は
永久に汚染される。** だから②は `captureUnsupported` を立ててページごと諦める作りになっていた。
＝ **中継が一度でもコケると、そのページの動くサムネが二度と復活しない。**

### 直し方

読み方が安全かを**別に持つ**（`feedable`）。素のURLへ倒したらそこで false にする。

```js
let feedable = feeding                                   // ON かどうか
...
⚠️ **ニコ生ページ側は元から無事だった。** あちらは中継を使わず `crossOrigin` で読み、失敗したら
`onerror` で**別の Image**（`plain`）を作って表示だけ確保する。給餌には渡していない。
**同じ「表示だけ守る」でも、片方は別インスタンス・片方は同じインスタンスの使い回しだった。**

### あわせて直した2つ

🔴 **汚れた canvas を捨てる。** 抱え込むと、原因が直った後もきれいな画像で例外が出続け、
「直したのに直らない」になる。

⚠️ **警告文を書き直した。** 旧文面は「本来CORS OK確認済みなので通常は起きない」と書いてあり、
実際に起きた時に**調べる人を確実に空振りさせる**（項目BK の「IndexedDB を疑え」と同じ失敗）。
心当たり（呼び出し側が安全でない読み方をしている）と復帰方法（ページを開き直す）を書いた。

### 検査（CV群）

実際に `updateThumbnailsFromStorage` を回して、給餌されたかどうかを見る:

- 中継が data URL を返せた → 給餌する（コマ化が止まらない）
- 🔴 中継が **null** を返した → 給餌しない
- 🔴 中継が **例外** を投げた → 給餌しない
- 中継が無い（ニコ生ページ）→ `crossOrigin='anonymous'` で読んで給餌する
- 汚染したら共有 canvas を捨てる

修正前の形に戻すと、真ん中の2つが鳴る（確認済み）。

---

## ✅ CW. 「よく見る順の履歴」のリセット（2026-08-12・利用者要望）

視聴履歴（配信者ごとの点数）を全部消す導線。`clearWatchHistory()` は 2026-08-10 から在ったが
**どこからも呼ばれていなかった**（「設定からの導線用」と書いたまま作っていなかった）。

⚠️ **画面では「視聴履歴」と呼ぶ**（利用者判断）。**「視聴ポイント」は自作用語**で、
   画面には一度も出てこない（テスト表示の `7pt` は 1.20.4 で撤去済み・項目CM-2）。説明文で初めて出す言葉にしない。「記録」は何の記録か伝わらず、「データ」は
   拡張の文脈だと「設定も含む全部」と読まれかねない ―― **消すのが怖くなる曖昧さ**。
   「履歴」は利用者が既に持っている言葉で、溜まって消せるものだと説明なしで伝わる。
   実質そのとおり視聴履歴なので、そう呼ぶほうが誠実でもある。

### 置き場所と形（利用者指定）

**「表示順序」の枠の中**に、小さく目立たなく置く。独立したセクションにはしない。
**「よく見る順」を選んでいる時だけ出す**（`.opt-recommend-only`）。

⚠️ **人気順では出さない。** 履歴は人気順の並びに一切効かないので、出すと関係が誤解される。
   （旧「人気順の基準」は人気順にも効いたので `active || recommend` だった。**条件が違う。**）
⚠️ **切り替えた時に出し分けを更新すること。** 保存だけして放置すると、新着順に変えても出たままになる。

🔴 **`type="button"` にすること。** 既定は submit なので、フォームが送信されて設定パネルが
閉じる／ページが再読み込みされる。

🔴 **2段階。** 取り消せない操作で救済が無い。1回目は「本当に消す（N人ぶん）」へ変わるだけで、
6秒放置すると戻る。履歴が0件なら「履歴はまだありません」で終わる。
**状態はボタン自身の文言で出す**（小さく置くために別枠を持たない）。
⚠️ **確認中だけ目立たせること**（`.is-armed`）。普段が控えめな見た目なので、
   何も変わらないと「押したのに反応が無い」に見えるし、押し間違いにも気付けない。

🔴 **設定ではなく「操作」。** `optionKeys` にも `saveOptions` にも関わらない。
足すと「保存される設定」になってしまう（保存するものが無い）。

### 🔴 `display` を当てたら `[hidden]` の打ち消しも書く（利用者報告で発覚）

「新着や人気順の時も消えません」。原因は CSS。

```css
#optionContainer .opt-recommend-only { display: flex; }   /* ← これが勝つ */
```

ブラウザ標準の `[hidden] { display: none }` は**クラス指定より弱い**ので、
**JS が `hidden` を立てても消えない。**

⚠️ **同じ罠の注意書きが main.css の `#kick_notice` に既にあった**（2026-08-10 に一度踏んでいる）。
読まずに同じ書き方をした。`.service_tabs[hidden]` も同じ理由で存在する。

検査を足した。**HTML から `hidden` 付きのクラスを集めて総当たり**し、`display` を当てているのに
`[hidden]` の打ち消しが無いものを落とす。新しいクラスでも自動で効く。

### 🔴 消した後の並べ替えを、リセット側から呼ばないこと

`chrome.storage.local.remove` は **自分のタブを含む全コンテキスト**で `storage.onChanged` を
起こす（削除は `newValue` が undefined で通知される）。`startWatchHistorySync` が既にそれを
受けてメモリを空にし、並べ替え直しまでやる。

ここで別に `rerankInPlace` を呼ぶと、**自分のタブだけ2回走る**うえ「同じことをする道が2本」に
なる。片方だけ直す事故が起きるので、別タブと同じ1本に寄せてある。

### 検査（CW群）

実物のボタンを `fire('click')` して動かす:
- 🔴 1回目では消えない／1回目は何人ぶんか出る／2回目で消える
- 🔴 消した後に並べ替え直す（`storage.onChanged` 経由で走ること）
- 🔴 リセットから直接 rerank を呼んでいない／履歴を消す実装が1つだけ
- `type="button"` である／`optionKeys` に足していない
- 🔴 表示順序の枠の中にある／よく見る順の時だけ出る／切り替えで出し分けが更新される
- 確認中だけ目立つ（`is-armed`）

⚠️ **偽DOMに `getElementsByName` が無くて落ちた。** 本物にあるものは用意しておくこと
（`updateCheckedState` が使う）。実装ではなく土台のせいで落ちる形だった。

⚠️ **「まっさらな状態」を前提にして空振り防止が鳴った。** `watchHistory` はモジュール内に
状態を持ち、`loadWatchHistory` は `loaded` フラグで2回目以降を素通りする。前の検査群が
読み込み済みなので、storage を差し替えても反映されない。**自分で足した値で判定する**形へ直した。

---

## ✅ 文言の整理（2026-08-12・利用者指定）

- 同接のヘルプから「Kick は実測値です。」を落とした。**設定の説明に要らない情報**で、
  Kick を使っていない利用者には意味が無く、使っている利用者にも行動が変わらない。
- 経過時間の「?」を外した。設定名だけで伝わる（カードの大きさと同じ判断）。

---

## ✅ CX. サムネ右下の経過時間（2026-08-12・利用者要望）

放送が始まってからの時間をサムネの右下に出す。設定「経過時間」で ON/OFF。**既定は OFF**
（更新しただけで既存利用者の見た目が変わらないようにする。同接の表示と同じ扱い）。

⚠️ **β版にはしていない。** 同接は推定値なので β を付けたが、こちらは開始時刻の引き算で
正確に出る。同じ「サムネの隅の表示」でも、確からしさが違う。

### 作り（同接の表示と対）

| | 場所 | 印 |
|---|---|---|
| 同時視聴者数 | サムネ**左上** | `<html>` の `nns-show-viewers` |
| 経過時間 | サムネ**右下** | `<html>` の `nns-show-elapsed` |

判定と印付けは `src/ui/elapsedTime.js` の1箇所（`ui/placement.js` / `ui/viewerCount.js` と同じ形）。
🔴 **印の向きは「出す時に付ける」。** 配線が動かなくても出ない側へ倒れる。
⚠️ **`pointer-events: none` は必須。** サムネは `<a>` の中なので、覆うとリンクが押せなくなる。
⚠️ 中身が空の時は `:empty` で枠ごと消す（開始時刻が無い番組で黒い箱だけ残さない）。

### 🔴 分より細かくしない

秒まで出すと毎秒 DOM を書き換えることになる。サムネの隅の数字にその価値は無い。
`23分` / `1時間23分` の2通りだけ。

⚠️ **開始が未来なら空**（予約枠・時計のずれ）。「-3分」を出さない。

### 🔴 生成時に自分で埋めること（利用者報告「表示までけっこう時間がかかる」）

カードを作る順番はこうなっている:

```
1. カードの箱を作る
2. applyRankAttributes(container, data)   ← ここで書こうとしても枠がまだ無い
3. サムネを作る
4. 経過時間の枠を append               ← 空のまま
```

**`applyRankAttributes` はカード生成の先頭で走る。** あとから append するバッジには、
その時点では何も書けない。生成時に自分で埋めないと、ticker（30秒）か次のリスト更新
（最大120秒）まで**空欄のまま画面に出る**。

⚠️ **同接バッジと視聴点数バッジには同じ注意書きが既にある**（「生成時は applyRankAttributes が
   これより前に走っているので、ここで一度埋める」）。すぐ上の行にあるのに同じ失敗をした。
   **サムネの隅に何か足す時は、必ず生成時にも埋める。**

検査は実物の `makeProgramElement` で1枚作り、その場で中身が入っているかを見る。
埋め込みを外すと空文字で落ちる（確認済み）。

### 🔴 ticker が要る理由

リストの取得は既定120秒間隔で、`applyRankAttributes` もその時にしか走らない。
**そのままだと最大2分ずれた時間が出る**（見ていると「止まっている」ように見える）。
`startElapsedTicker()` が 30秒ごとに DOM の `data-begin-at` から書き直す。

- **取得も保存もしない。** 文字列を書くだけ。だから拡張が無効化された後も止めていない
  （項目BK が問題にしたのは「取得が止まらない」こと。ここは通信も storage も触らない）。
- **設定が OFF の間も回す。** 出し分けは CSS が持っており、ここで設定を見に行くと
  「設定を読む経路」が増える。書くのは文字列だけなので止める価値が無い。
- **両ページが1回だけ呼ぶ。** 呼び忘れると、そのページだけ時間が止まって見える（検査 CX が縛る）。
- 書き手は `writeElapsedLabel` の1つだけ。描画時と ticker の両方がここを通る
  （増やすと2つの経路で食い違う）。

### 検査（CX群）

設定値の解釈／文字列（分・時間分・0分・未来は空・壊れた値は空）／印の向き（実際に呼ぶ）／
両ページが反映と ticker を呼ぶ／書き手が1つ／ticker が取得しない・二重に張らない／
設定 UI（保存・復元・必須ガードに入れない・optionKeys に載っている）／
CSS（既定非表示・印が付いた時だけ表示・pointer-events・:empty・置き場所）。

壊して鳴ることを確認済み: 未来を許す／kick 側の ticker を消す／`pointer-events` を外す。

---

## ✅ CY. 番組カードの設定をひとまとめにする（2026-08-12・利用者指定）

設定が増えて平らに並びすぎたので、**カードの見た目にかかる4つ**を1つの枠のサブ項目にした。
...
番組カードの設定
  ...
  │ 同時視聴者数 β版 ? [OFF][ON]
  │ 経過時間 ?        [OFF][ON]
サイドバーの置き方／テーマ／Kick 連携 β版
```

置き場所は**自動移動の下**（利用者指定・2026-08-12）。
⚠️ 同じ日に「同時視聴者数を表示順序の下へ」→「グループごと表示順序の直後へ」→
   「自動移動の下へ」と3回動いている。**置き場所は利用者が実機で見て決めるもの**で、
   こちらの理屈で動かさないこと。

サブ項目は**小さく・細く・薄く**（0.88em / normal / opacity .7）。
🔴 **見出しだけ小さくしても「サブ」に見えない。** セグメントのボタン・β版バッジ・「?」も
   一回り小さくしてある。検査 CY が大きさ・太さ・薄さ・ボタンの4つを見る。

### 🔴 まとめ直しで壊れるのは「見た目」ではなく「保存」

`name` と `value` が動くと、保存済みの設定に対応するラジオが無くなり、
**その利用者は設定を一切保存できなくなる**（項目BQ の罠）。
`updateCheckedState` がどれも選ばない状態にし、`saveOptions` が「1つも選ばれていない」で
早期 return するため。**移動しただけのつもりで value を整えたくなるが、触らないこと。**

検査 CY が4つの `name` と `value` の並びを丸ごと固定している。実際に `medium` → `normal` に
変えると CY だけでなく既存の BS も3項目落ちる（確認済み）。

### 階層の見せ方

- サブ項目は `.opt-subsection`（左に細い線＋一段下げ）
- 見出しは `.opt-sublabel`（枠の `.opt-label` より小さく、太字にしない）
  🔴 **同じ大きさにしないこと。** 階層が読めず、まとめた意味が無くなる。
- 🔴 **ヘルプ付きのサブ項目は `opt-title-with-help` を保つこと。**
  吹き出しは `left:0; width:100%` で**いちばん近い positioned 祖先**に合わせて出る。
  外すと別の箱の幅で出て右にはみ出す（項目CW で `.opt-recommend-only` に
  `position: relative` を足したのと同じ話）。

### 🔴 ボタンの幅は「見出しの列幅」で揃える（2026-08-13・利用者指定）

サブ項目は `[見出し][ボタン]` の1行。見出しを `flex: 0 0 auto` にしていたので、
**見出しの長さぶんボタンの左端がずれ**、右端だけが揃った不揃いな並びになっていた。

実測（Chrome・サイドバー幅360px・ボタン枠の幅）:

```
                  変更前              変更後
カードの大きさ      222px             172px
動くサムネ β版 ?    188px             172px
同時視聴者数 β版 ?  177px             172px
経過時間           257px             172px   ← 見出しが短いぶん 80px も広かった
```

直し方は**見出しの列幅を固定する**（`.opt-sublabel { flex: 0 0 132px }`）。
ボタン側の `flex: 1 1 130px` は触らない。左端が揃えば、右端は元から揃っているので幅も揃う。

- ⚠️ **132px はいちばん長い見出しの実測値（127px）から決めた数字。**
  縮めると「同時視聴者数 β版 ?」が2行になる。検査 CY が 130px を下限に見ている。
- ⚠️ 見出しを増やす・長くする時は**測り直すこと**。目で決めない。
- ⚠️ 1行に収まるのは `132 + gap 10 + 130 = 272px` 以上＝**サイドバー幅 304px 以上**。
  既定の360pxは1行。それより狭いと4行とも折り返す（以前は経過時間だけ折り返さず、
  そこだけ横並びのまま残っていた）。

### 🔴 doc/12 の書き出しが3件静かに減った（同じ失敗の2度目）

サブ項目を `opt-sublabel` にした結果、`scripts/dump-help-texts.mjs` が
`opt-label` しか見ておらず、**動くサムネ・同時視聴者数・経過時間の3件が doc/12 から消えた**。

⚠️ **そのファイルの冒頭に「取りこぼしても静かに減るだけで、この設定にはヘルプが無いと
   誤って読める」と書いてある**（2026-08-10 に「テーマ」で1度踏んでいる）。読まずに再演した。

走査を直すだけでは同じことがまた起きるので、**歯止めを入れた**:

- 実装の `help-tooltip` の数と、書き出した数を突き合わせる。合わなければ**落とす**
- 見出しを持たないヘルプ（ボタンの隣に置いた「履歴をリセット」）も拾う

走査を元に戻すと `NG 実装に 11 件のヘルプがあるのに 8 件しか書き出せていない` で落ちる（確認済み）。

### ⚠️ コメントが実物から離れていることがある

「カードの大きさ」の説明コメントが、**サイドバーの置き方のセクションの直前**に置かれていた。
コメントを起点に切り出したら隣のセクションを掴んだ（1度踏んで戻した）。
**切り出しはそのセクション自身の中身（`name="..."` など）で特定すること。**
迷子のコメントはグループの説明へ畳んだ。

---

## ✅ CZ. 同時視聴者数と経過時間が、読み込みによっては全番組で出ない（2026-08-17・利用者報告）

**症状**: 「同時視聴者数と経過時間を両方ONにしているのに、表示されない場合がある。**全番組**で。
ページの読み込みのタイミングで出たり出なかったり。出るほうが多い。」

### 症状の形が原因を絞る

この2つの表示は**カードごとに出し分けていない**。オーバーレイは常に作られていて、
見えるかどうかは `<html>` のクラス1つで決まる（項目CX の表）。

| 表示 | 印 |
|---|---|
| 同時視聴者数 | `<html>` の `nns-show-viewers` |
| 経過時間 | `<html>` の `nns-show-elapsed` |

印を付けるのは `main.js` の `reflectOptions()` の2行だけで、渡すのは `options.showViewerCount` /
`options.showElapsedTime`。**どちらの既定も OFF。**
だから「両方が・全番組で・一斉に」出ないのは、`options` が既定に落ちた時**しか**起きない。

### 原因: 書き戻しの失敗で、読めていた設定を捨てていた

`getOptions` は「読み取り」と「既定値の焼き付け（書き戻し）」を**同じ `try` に入れていた**。

```js
// 修正前
try {
    const stored = await get(...)              // ← ここは成功している
    const merged = migrateOptions({ ...defaultOptions, ...stored })
    await set(pickOptionKeys(merged))          // ← ここがこけると
    return merged
} catch (error) {
    return defaultOptions                      // ← 読めていた設定ごと捨てる
}
```

書き戻しは**次回を速くするための副作用**であって、戻り値の正しさとは関係が無い。
`try` を2つに分けて、書き戻しの失敗は記録するだけにした。

🔴 **既定へ倒してよいのは「読めなかった時」だけ。** そこは従来どおり残してある（検査 CZ で固定）。

⚠️ **放っておくと恒久化しうる欠陥だった。** `reflectOptions()` は `setupOptionsHandler(options, …)`
も呼ぶので、既定に落ちた `options` で設定フォームのラジオが OFF に描かれる。
その状態で利用者が**別の設定を1つ触るだけ**で `saveOptions` が両方 OFF を保存する。
「再読込すれば直る」で済んでいたのは、たまたまその操作をしていなかったから。

### 🔴 静かに倒さないこと

読み取りに失敗して既定へ倒す時は、**症状を名指しした警告**を出す。
「設定が全部その日だけ効かない」は利用者から原因が絶対に分からない種類の壊れ方で、
`handleError` の技術的なログだけでは繋がらない。

### 調べて**外した**筋（記録として）

| 疑ったもの | 結果 |
|---|---|
| `<html>` のクラスを誰かが `className =` で上書きしている | **無い。** 全箇所 `classList.toggle/add/remove` |
| 設定キーが `storage.local.remove` で消える → onChanged が undefined を配る | **無い。** 消しているのは `AUTO_NEXT_*` と視聴履歴だけ |
| content script が DOMContentLoaded に間に合わない | **無い。** `run_at` は `document_start` |
| オーバーレイの中身が空 | 経過時間は `:empty` で消えるが、同接は「計算中」を出すので**両方同時**にはならない |

### 検証

`verify:loop` の項目CZ（5件）。**実際に `set` を失敗させて戻り値を確かめる**（正規表現で見ない）。
書き戻し失敗で保存値が返ること・その値で「出す」と判定されること・他の設定も生きていること・
読み取り失敗の時だけ既定へ倒ること。

壊して鳴ることを確認済み（書き戻しの catch に `return defaultOptions` を戻すと3件が NG）。

⚠️ **スタブは `get(callback)` の1引数で呼ばれる。** 第2引数を前提に書いて全項目が
「前提」から落ちた（この日に踏んだ）。
⚠️ **`BQ 移行は…getOptions を通る` が巻き添えで落ちた。** `const merged = migrateOptions(` と
綴っていたため、宣言を `let` へ変えただけで NG になった。**宣言の書き方を検査に含めないこと。**

### 🔴 鎖の真ん中が誰も見ていなかった（敵対的レビューで発見）

設定が画面に届くまでは3つに分かれている。**両端だけ固定して、真ん中が空いていた。**

| 鎖 | 固定していた検査 |
|---|---|
| `getOptions` が正しい値を返す | CZ（この項目） |
| `applyShow*()` が `<html>` に印を付ける | CX / CZ |
| **その2つを繋ぐ `applyShow*(options.showX)` の1行** | **どこにも無かった** |

`main.js` を `applyShowElapsedTime("off")` に壊しても**全項目が合格した。**
CX は「その関数を呼んでいるか」しか見ておらず、**何を渡しているか**を見ていなかった。
利用者が報告した症状そのものの形なのに素通りする状態だった。

検査を足したが、⚠️ **形を細かく縛らないこと。** 最初 `fn(<変数>.<設定名>)` の完全一致で書いたら、
`fn(changes.showElapsedTime.newValue)` のような**正当な書き方まで NG** になった。
捕まえたいのは直書きなので、見るのは「リテラルを渡している呼び出しが0」と
「設定名のプロパティから採っている呼び出しが1つ以上」の2つだけにしてある。

🔴 **教訓: 両端を固定しても鎖は保証できない。** 「設定 → 画面」のように段が分かれている機能は、
**段の継ぎ目そのもの**を1本縛ること。

---

## ✅ DA. Kick連携のお知らせ（2026-08-17・利用者要望）

**発端**: 「Kick連携は 1.13.0 で入れたのに、設定の下の方にあるので**存在に気づいていない利用者が
いるのではないか**。設定アイコン付近に『Kick連携機能が追加されました』のようなものを出したい。
利用者の意思で消せて、一度消したらもう出ないようにしたい。」

### なぜ画面に出すしかないか

**既定値を変えて知らせることはできない**（doc/06）。`getOptions` が `{...defaults, ...stored}` を
書き戻すので、既存利用者の storage には既にそのキーの既定が焼き付いている。
**届いてほしい既存利用者にだけ届かない。** 残るのは「画面に出す」か「ストアの更新内容」だけ。

導線が4手あるのも理由になっている: サイドバーを開く → 設定を開く → 下までスクロール →
拡張の設定ページへ移動。**下まで見た人しか存在を知り得ない。**

### 決めたこと

| | |
|---|---|
| 出す相手 | **Kick を有効にしていない人だけ。** 知っている人に知らせても邪魔なだけ |
| 出す場所 | 設定アイコンの真下（`.sidebar_header` の中に幅100%の行として折り返させる） |
| 消える条件 | ①×を押す ②案内の「設定を見る」を押す ③Kick を有効にする |
| 消えない条件 | **歯車を普通に押しただけでは消さない。** 見落とした人に二度と出なくなる |
| 出すページ | **ニコ生のみ。** kick.com のサイドバーは連携が有効な時しか出ない＝知らせる相手が居ない |

🔴 **これは「両ページへ配線する」決まり（項目BN）の意図的な例外。**
写し漏れと誤解されないよう、検査 `DA` で**逆向きに固定**してある（kickPage に配線が無いこと）。

### 🔴 保存キーを `optionKeys` に入れないこと

`kickIntroDismissed` は**設定ではなく一度きりのUI状態**。`optionKeys` に入れると
`getOptions` が毎回書き戻す対象になり、`saveOptions` の巻き添えも受ける。
入れなければ「**キーが在る＝消した**」で済み、既定値を持たせる必要すら無い。

### 🔴 壊れ方は「出ない」側へ倒す

失敗の方向が非対称。**「一度消したのにまた出る」は「消しても消えない拡張」に見える**ので、
分からない時は出さない。

- マークアップの既定は `hidden`。出すのは JS が外した時だけ
- `storage` が読めなければ「消した」扱い
- SW が答えなければ「Kick 有効」扱い
- `shouldShowKickIntro` は `dismissed === false && kickGranted === false` と**厳密に**書く。
  `!state.dismissed` にすると `undefined` が「未読」になって出てしまう（検査で鳴る）
- `storage.onChanged` は**消す方向にしか動かさない**（`newValue` が真の時だけ）。
  逆向きにすると storage が壊れた時に消したはずの案内が復活する

### 作り

| ファイル | 役割 |
|---|---|
| `src/ui/kickIntro.js` | 保存キー・出す条件・読み書き。**判断はここだけ** |
| `src/render/sidebar.js` | マークアップと `setKickIntroVisible()`（**印を付けるだけ**） |
| `src/main.js` | `reflectKickIntro()` で判断して反映、ボタン3つの配線、別タブ追従 |
| `src/styles/main.css` | `#kick_intro` 一式 |

⚠️ **絶対配置にしない。** `.sidebar_header` は flex なので幅100%の行として折り返させれば、
設定パネルにもリストにも重ならない。重ねると z-index と「設定を開いている間は隠す」処理が要る。
⚠️ **`#kick_intro[hidden] { display: none }` が要る。** flex の子は `hidden` 属性だけでは消えない。
⚠️ 上下の余白は `padding` で持つ。`margin` だと `.sidebar_header` の下の余白と**相殺**する（131行目と同じ罠）。

### 検証

`verify:loop` の項目DA（17件）。壊して鳴ることを4通り確認済み:
`shouldShowKickIntro` を緩い判定へ／読めない時を「出す」側へ／既定の `hidden` を外す／
歯車でも消えるようにする。

### 🟡 まだ決まっていない

**文言と置き場所は実機で見てから直す前提。** 過去に「番組カードの設定」の置き場所が
同じ日に3回動いている（項目CY）。**理屈で動かさず、見てもらって決めること。**

---

## ✅ DB. kick.com でニコ生の縦型配信だけサムネが出ない（2026-08-17・利用者報告・再現条件あり）

**症状**: 「サムネが配信者アイコンのままで、ホバーするとその上に過去サムネがアニメーションする。
リロードすると治る。**縦映像。**」→ 追加報告で **「kick.com ページ側で起こる。条件は縦型サムネ」**
と再現条件が判明した。
...
| 見えていたもの | 理由 |
|---|---|
| サムネがアイコンのまま | **kick.com には詳細APIでの補完が無い**（下記）。縦型は補完でしか埋まらなかった |
| ホバーで過去サムネが動く | 動くサムネのコマは **SW の IndexedDB**（オリジン非依存）。ニコ生ページで貯めた分が kick.com でも再生される |
| リロードで治る | **それはニコ生ページでの話**。あちらは補完が走るので、その回に成功すれば出る |

### 原因: 補完がニコ生ページにしか無い（項目BN の型の再発）

`fillMissingDetails` の呼び出しは **`fetchFollowedProgramsViaPage` の1箇所だけ**＝ニコ生ページ専用。
kick.com は SW から生データを受け取って `mapApiProgramToInfo` に通すだけ（`kickPage.fetchNicoPrograms`）。

🔴 **しかも kick.com からは補完のしようが無かった。** 詳細API `api.cas.nicovideo.jp` は
**manifest に一切書かれていない**（optional_host_permissions にも無い）。CORS も
`https://live.nicovideo.jp` にしか開いていない（項目BF-2 の実測）。
つまり「kick.com でも補完する」には**新しいホスト権限が要る**。

### 直し方: 包みを解く（権限を増やさずに済んだ）

実測（2026-08-17・公開の recent 版70件）:

| 一覧APIの形 | 件数 | 中身 |
|---|---|---|
| 素のスクショ | 47 | そのまま採用 |
| `?image=prod-lv…` の包み（固定画像運用） | 20 | `flipped` から回収（既存の道） |
| **`?url=<エンコードしたURL>` の包み** | **3** | **解くと `…/screenshot/…/thumbnail-360x640/…`＝縦型** |

**縦型のスクショは、必ずこの `?url=` の包みで来る。** 解けば詳細APIは要らない。

```
mapApiProgramToInfo → pickLiveScreenshot(listingThumbnail, flipped)
    ① そのまま isLiveScreenshotUrl を通るか
    ② 通らなければ unwrapListingThumbnail で中身を取り出し、**同じ判定にもう一度かける**
```

実測で **写像後にサムネが空になる番組が 3件 → 0件** になった。
`mapApiProgramToInfo` は両ページが通る共有関数なので、**kick.com もニコ生も同時に直る。**
副次的に、ニコ生ページの `fillMissingDetails` の対象も減る。

### 🔴 これは項目AA の「ホストで緩める」ではない

同じ実測に **中身が `img.cdn.nimg.jp/…/128x128l…`（チャンネルアイコン）の包みが1件**あった。
ホストで通していたらこれをライブサムネとして登録していた（項目AA の事故そのもの）。
**取り出した中身を `isLiveScreenshotUrl` にもう一度かける**ので、これは落ちる。判定は緩めていない。

⚠️ ホスト名は**完全一致**で見ること（部分一致は項目CB の誤爆と同じ）。
⚠️ 採用の判定を `pickLiveScreenshot` の外に書かないこと。書き手が2人になると項目AA が再発する。

### 🟡 残っている穴（kick.com のみ）

**放送直後でスクショ自体が未生成の番組**は、一覧APIが何も返さないので kick.com では埋まらない
（ニコ生ページは補完で埋まる）。スクショができ次第、次の周期で出る＝一時的。
直すなら `api.cas.nicovideo.jp` を optional_host_permissions へ足して SW 中継にするが、
🔴 **`hasKickPermission()` は集合を丸ごと照合する**ので、足すなら `KICK_REQUEST_PERMISSIONS`
だけに入れて別途 `hasCasPermission` で見ること（live2 と同じ形・項目CT）。
**既存の Kick 利用者は OFF→ON し直すまで権限が付かない。** 一時的な症状のために払う代償としては重い。

### 検証

`verify:loop` の項目DB（17件）。壊して鳴ることを3通り確認済み（包みを解くのをやめる／
ホストで緩める／解いた中身を再判定せずに採用する）。

🔴 **既存の検査3つが「古い方針」を固定していたので、意図を保ったまま土台を替えた。**
書き換えて通しただけにしないこと。何を守っていたかを先に読むこと。

| 検査 | 何を守っていたか | どう替えたか |
|---|---|---|
| `AW 包まれた形は採用しない` | 項目AA の事故（アイコンをサムネ扱い） | **包みの中身がスクショでない**場合の拒否に替え、採用側も併記 |
| `BK flipped の形が変わったら鳴る` | 罠が本当に鳴ること | 「回収できない形」の代表を**包まれたアイコン**に替えた（スクショのままだと回収できて空振り） |
| `CJ ① スクショ未生成の間はアイコン` | 補完が1回失敗しても巻き戻らないこと（③が本題） | 土台を「包まれた縦型」→「**listingThumbnail が空**」に替えた |
