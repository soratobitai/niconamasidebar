...
## 🔴 CM-2. テスト用の表示（2026-08-10）

実機で数字を確かめるために出していたもの。**片方は正式な設定になった**（項目CR）。

| 表示 | 場所 | 今の扱い |
|---|---|---|
| 視聴点数（`7pt`） | 配信者名の行の右端 | 🔴 **リリース時に撤去。** 消す箇所は下の表 |
| 同時視聴者数 | サムネ左上 | ✅ β版の設定「同時視聴者数」になった（項目CR） |

`7pt` を消す箇所: `applyRankAttributes` の2行 / `makeProgramElement` のバッジ生成 /
`main.css` の `.watch_count_badge`。

どちらも**順位に使うのと同じ値**を出している（表示と並びがずれない）。
